...
public class VentanaRegistro extends JDialog {
	 ...
	 * Create the frame.
	 */
	public VentanaRegistro(Pedido pedido, VentanaPrincipal ventPrin) {
	...
	public VentanaPrincipal getVentPrin() {
		return ventPrin;
	}
	...
	private void compruebaLongitudPassword() {
		if(getPwdPassword().getPassword().length < 8) {
			JOptionPane.showMessageDialog(null, "ERROR: La contraseña no tiene 8 caracteres");
			getPwdPassword().grabFocus();
			getPwdPassword().selectAll();
		}
	}
	
	private void mostrarVentanaConfirmacion() {
		if(getRdbtnLlevar().isSelected()) {
			pedido.setLocal(false);
		}
		VentanaConfirmacion frame = new VentanaConfirmacion(pedido, this);
