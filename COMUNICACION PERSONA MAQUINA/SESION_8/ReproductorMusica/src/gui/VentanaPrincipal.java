...
public class VentanaPrincipal extends JFrame {
	...
	private void play() {
		...
		musicPlayer.play(listPlay.getSelectedValue());
	}
		...
		if(respuesta == JFileChooser.APPROVE_OPTION) {
			for(int i = 0; i < selector.getSelectedFiles().length; i++) {
				...
				for(int j = 0; j < modelolistLibrary.size(); j++) {
					if(modelolistLibrary.get(j).equals(selector.getSelectedFiles()[i])) {
						encontrado = true;
						break;
					}
				}
		...
		if (mntmAbout == null) {
			...
			mntmAbout.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					JOptionPane.showMessageDialog(null,"EII Music Player\nAplicacion para reproducir musica\nEloy Alfredo Schmidt Rodríguez UO271588","About EII Music Player",JOptionPane.PLAIN_MESSAGE,icono);
				}
			});
		}
		return mntmAbout;
